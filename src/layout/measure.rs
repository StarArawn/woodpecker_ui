...
impl Measure for ImageMeasure {
    fn measure(
        ...
        let s_width = style.size.width.maybe_resolve(parent_width, calc);
        let s_min_width = style.min_size.width.maybe_resolve(parent_width, calc);
        let s_max_width = style.max_size.width.maybe_resolve(parent_width, calc);
        let s_height = style.size.height.maybe_resolve(parent_height, calc);
        let s_min_height = style.min_size.height.maybe_resolve(parent_height, calc);
        let s_max_height: Option<f32> = style.max_size.height.maybe_resolve(parent_height, calc);
